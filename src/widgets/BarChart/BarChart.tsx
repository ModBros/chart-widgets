...
import {
  ...
  Loading,
  useIsMetricFieldConfigured, useCheckboxField, Color
...
const AnimatedBar: FunctionComponent<AnimatedBarProps> = (props) => {
  ...
  return <AnimatedVisxBar x={0} y={0} height='100%' rx={cornerRadius} ry={cornerRadius} style={styles} fill={color.toRgbaCss()}/>
}
...
const BarChart: FunctionComponent = () => {
  ...
  const backColor = useColorField({field: 'back_color', defaultColor: '#ffffff'})
  const maxValue = useNumberField({field: 'max'})
  const cornerRadius = useNumberField({field: 'corner_radius', defaultValue: 0});
  ...
  const value = parseFloat(channelValue?.value?.value?.toString());
  const max = getMetricMaxValue(channelValue, maxValue);
  const width = max !== 0 ? value * 100 / max : 0

  return (
    <svg width={'100%'} height={'100%'}>
      <Shape.Bar x={0} y={0} rx={cornerRadius} ry={cornerRadius} height='100%' width='100%' fill={backColor.toRgbaCss()}/>
